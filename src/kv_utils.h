...
struct KVLogHeader {
    // uint8_t  is_valid;
    uint8_t  ctl_bits; // valid, commit gc
    uint16_t key_length;
...
static inline bool log_is_valid(KVLogHeader * header) {
    return !!(header->ctl_bits & KV_LOG_VALID);
}

static inline bool log_is_committed(KVLogHeader * header) {
    return !!(header->ctl_bits & KV_LOG_COMMITTED);
}

static inline bool log_is_gc(KVLogHeader * header) {
    return !!(header->ctl_bits & KV_LOG_GC);
}

static inline bool log_is_insert(KVLogHeader * header) {
    return !!(header->ctl_bits & KV_LOG_INSERT);
}
