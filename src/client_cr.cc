...
IbvSrList * ClientCR::gen_log_commit_sr_lists(uint32_t coro_id, uint8_t ctl_bits, std::vector<KVRWAddr> & rw_addr_list, __OUT uint32_t * num_sr_lists) {
...
IbvSrList * ClientCR::gen_update_prev_sr_lists(KVReqCtx * ctx, __OUT uint32_t * sr_list_num) {
    IbvSrList * ret_sr_lists = (IbvSrList *)malloc(sizeof(IbvSrList) * num_replication_);
...
void ClientCR::free_update_prev_sr_lists(IbvSrList * sr_lists, int num_sr_lists) {
    for (int i = 0; i < num_sr_lists; i ++) {
        free(sr_lists[i].sr_list[0].sg_list);
        free(sr_lists[i].sr_list);
    }
...
void ClientCR::prepare_log_commit_addrs(KVReqCtx * ctx) {
    for (int i = 0; i < num_replication_; i ++) {
        ...
        // ctx->log_commit_addr_list.push_back(cur_rw_addr);
        // print_log(DEBUG, "!!!!!!commmit addr(%lx) server(%d) rkey(%x)", 
        //     cur_rw_addr.r_kv_addr, cur_rw_addr.server_id, cur_rw_addr.rkey);
    }
}
...
void ClientCR::modify_primary_idx(KVReqCtx * ctx) {
    ...
    // assert(ret == 0);
    // ib_free_sr_lists(pr_cas_sr_list, pr_cas_sr_list_num);
    // if (ctx->req_type == KV_REQ_UPDATE || ctx->req_type == KV_REQ_DELETE) {
    //     ib_free_sr_lists(invalid_sr_list, 1);
    // }
...
void ClientCR::kv_delete_read_buckets(KVReqCtx * ctx) {
    int ret = 0;
    KVLogHeader * header = (KVLogHeader *)ctx->kv_info->l_addr;

    // 1. generate send requests
    // print_log(DEBUG, "\t[%s] 1. read blocks", __FUNCTION__);
    uint32_t read_bucket_sr_list_num;
    IbvSrList * read_bucket_sr_list = gen_read_bucket_sr_lists(ctx, &read_bucket_sr_list_num);
    // kv_assert(read_bucket_sr_list != NULL);
...
void ClientCR::kv_delete_read_buckets_sync(KVReqCtx * ctx) {
    int ret = 0;
    KVLogHeader * header = (KVLogHeader *)ctx->kv_info->l_addr;
...
void ClientCR::update_kv_header(KVLogHeader * header, ClientMMAllocCtx * mm_alloc_ctx) {
...
int ClientCR::client_recovery() {
    ...
    // TODO: recover the last used mm_ctx
