...
int Client::kv_delete_sync(KVReqCtx * ctx) {
    // print_log(DEBUG, "[%s] start", __FUNCTION__);
    prepare_request(ctx);
    kv_delete_read_buckets_sync(ctx);
...
void Client::fill_heartbeat_addr(uint8_t server_id, __OUT KVRWAddr * hb_addr) {
...
IbvSrList * Client::gen_log_commit_sr_lists(uint32_t coro_id, uint8_t ctl_bits, std::vector<KVRWAddr> & rw_addr_list, __OUT uint32_t * num_sr_lists) {
    IbvSrList * ret_sr_lists = (IbvSrList *)malloc(sizeof(IbvSrList) * num_replication_);
    uint8_t * local_val_addr = (uint8_t *)malloc(sizeof(uint8_t));
    // *local_val_addr = (KV_LOG_VALID | KV_LOG_COMMITTED);
    *local_val_addr = ctl_bits;
    for (int i = 0; i < num_replication_; i ++) {
        struct ibv_send_wr * sr = (struct ibv_send_wr *)malloc(sizeof(struct ibv_send_wr));
        struct ibv_sge * sge = (struct ibv_sge *)malloc(sizeof(struct ibv_sge));
        ...
        sge->addr = (uint64_t)local_val_addr;
        sge->length = 1;
        sge->lkey = 0;
...
IbvSrList * Client::gen_update_prev_sr_lists(KVReqCtx * ctx, __OUT uint32_t * sr_list_num) {
    IbvSrList * ret_sr_lists = (IbvSrList *)malloc(sizeof(IbvSrList) * num_replication_);

    KVLogHeader virt_ptr;
    uint32_t log_header_next_offset = (uint64_t)&virt_ptr.next_addr - (uint64_t)&virt_ptr;
    for (int i = 0; i < num_replication_; i ++) {
        uint8_t r_server_id = ctx->mm_alloc_ctx.prev_addr_list[i] & 0xFF;
        struct ibv_send_wr * sr = (struct ibv_send_wr *)malloc(sizeof(struct ibv_send_wr));
        struct ibv_sge     * sge = (struct ibv_sge *)malloc(sizeof(struct ibv_sge));
        ...
        sge->addr   = (uint64_t)&ctx->mm_alloc_ctx.addr_list[0];
        sge->length = sizeof(uint64_t);
        sge->lkey   = 0;

        sr->wr_id = ib_gen_wr_id(ctx->coro_id, r_server_id, UPDATE_PREV_ST_WRID, i + 1);
        sr->sg_list = sge;
        sr->num_sge = 1;
        sr->opcode = IBV_WR_RDMA_WRITE;
        sr->send_flags = IBV_SEND_INLINE;
        sr->wr.rdma.remote_addr = (ctx->mm_alloc_ctx.prev_addr_list[i] ^ (uint64_t)r_server_id) + log_header_next_offset;
        sr->wr.rdma.rkey = server_mr_info_map_[r_server_id]->rkey;
        sr->next = NULL;

        ret_sr_lists[i].sr_list = sr;
        ret_sr_lists[i].num_sr = 1;
        ret_sr_lists[i].server_id = r_server_id;

        // print_log(DEBUG, "\t  [%s fb%d %ld] update prev sid(%d) addr(%lx)", __FUNCTION__,
        //     ctx->coro_id, boost::this_fiber::get_id(), r_server_id, sr->wr.rdma.remote_addr);
    }

    *sr_list_num = num_replication_;
    return ret_sr_lists;
}

void Client::free_update_prev_sr_lists(IbvSrList * sr_lists, int num_sr_lists) {
    for (int i = 0; i < num_sr_lists; i ++) {
        free(sr_lists[i].sr_list[0].sg_list);
        free(sr_lists[i].sr_list);
    }
...
void Client::prepare_log_commit_addrs(KVReqCtx * ctx) {
    for (int i = 0; i < num_replication_; i ++) {
        ...
        cur_rw_addr->r_kv_addr = ctx->mm_alloc_ctx.addr_list[i];
        cur_rw_addr->server_id = ctx->mm_alloc_ctx.server_id_list[i];
        cur_rw_addr->rkey = ctx->mm_alloc_ctx.rkey_list[i];

        // print_log(DEBUG, "[%s fb%d %lld] commmit addr(%lx) server(%d) rkey(%x)", __FUNCTION__, ctx->coro_id, 
        //     boost::this_fiber::get_id(), cur_rw_addr->r_kv_addr, cur_rw_addr->server_id, cur_rw_addr->rkey);
    }
}
...
void Client::modify_backup_idx_consensus_1(KVReqCtx * ctx) {
    ...
    if (ctx->consensus_state == KV_CONSENSUS_FAIL) {
        // print_log(DEBUG, "\t  [%s fb%d %ld] 2. return failed finished", __FUNCTION__, ctx->coro_id, boost::this_fiber::get_id());
    ...
    uint32_t bk_hb_sr_list_num;
    IbvSrList * bk_cas_sr_list_1 = gen_cas_sr_lists(ctx->coro_id, ctx->kv_modify_bk_1_cas_list, &bk_cas_sr_list_1_num);
    IbvSrList * bk_hb_sr_list = gen_write_hb_sr_lists(ctx->coro_id, ctx->kv_modify_bk_1_heartbeat_list, &bk_hb_sr_list_num);
    ...
    kv_insert_p2_sr_list_batch.push_back(bk_hb_sr_list);
    kv_insert_p2_sr_list_num_batch.push_back(bk_cas_sr_list_1_num);
    kv_insert_p2_sr_list_num_batch.push_back(bk_hb_sr_list_num);
    ...
    free_write_hb_sr_lists(bk_hb_sr_list, bk_hb_sr_list_num);
...
void Client::modify_primary_idx(KVReqCtx * ctx) {
    ...
    if (ctx->consensus_state == KV_CONSENSUS_FAIL) {
        ...
        if (ctx->req_type == KV_REQ_INSERT) {
            ...
            mm_->mm_free_key_all(ctx->key_str);
        }
        return;
    }
    ...
    if ((ctx->req_type == KV_REQ_UPDATE || ctx->req_type == KV_REQ_DELETE)) {
        ...
        uint8_t * is_valid = (uint8_t *)malloc(sizeof(uint8_t));
        *is_valid = KV_LOG_COMMITTED;
    ...
    // print_log(DEBUG, "\t  [%s fb%d %ld] checking result (real: %lx, expected: %lx)", __FUNCTION__, ctx->coro_id, boost::this_fiber::get_id(),
    //     *(uint64_t *)ctx->kv_modify_pr_cas_list[0].l_kv_addr, ctx->kv_modify_pr_cas_list[0].orig_value);
    ...
    if (*(uint64_t *)ctx->kv_modify_pr_cas_list[0].l_kv_addr != ctx->kv_modify_pr_cas_list[0].orig_value) {
        if (ctx->req_type == KV_REQ_INSERT) {
            ...
            ctx->ret_val.ret_code = KV_OPS_SUCCESS;
            ctx->is_finished = true;
            // assert(*(uint64_t *)ctx->kv_modify_pr_cas_list[0].l_kv_addr == 0);
        }
        return;
    }
...
void Client::modify_primary_idx_sync(KVReqCtx * ctx) {
    ...
    // check update success
    // print_log(DEBUG, "\t  [%s] checking result (real: %lx, expected: %lx)", __FUNCTION__,
    //     *(uint64_t *)ctx->kv_modify_pr_cas_list[0].l_kv_addr, ctx->kv_modify_pr_cas_list[0].orig_value);
    ...
    // if (ctx->req_type == KV_REQ_DELETE) {
    //     assert(*(uint64_t *)ctx->local_cas_target_value_addr == 0);
    // }
...
bool Client::check_cas_consensus_is_no_win(const std::map<uint64_t, int> & win_addr_map) {
    std::map<uint64_t, int>::const_iterator it;
    int cnt = 0;
    for (it = win_addr_map.begin(); it != win_addr_map.end(); it ++) {
        cnt += it->second;
        if (it->second > (num_replication_ - 1) / 2) {
            return false;
        }
    }
    // assert(cnt == num_replication_ - 1);
    return true;
}
...
void Client::check_cas_consensus_0(KVReqCtx * ctx) {
    ...
    uint64_t target_addr = HashIndexConvert40To64Bits((*(RaceHashSlot *)&target_value).pointer);    // the address in the target
    uint64_t min_target_addr = target_addr;     // min address among all targets
    ctx->kv_modify_bk_1_cas_list.clear();
    ctx->kv_modify_bk_1_heartbeat_list.clear();
    // uint32_t num_success = 0;
    std::map<uint64_t, int> win_addr_cnt;
    for (size_t i = 0; i < ctx->kv_modify_bk_0_cas_list.size(); i ++) {
        // kv_assert(ctx->kv_modify_bk_0_cas_list[i].orig_value == expected_value);
        // kv_assert(ctx->kv_modify_bk_0_cas_list[i].swap_value == target_value);
        uint64_t swap_back = *(uint64_t *)ctx->kv_modify_bk_0_cas_list[i].l_kv_addr;
        // print_log(DEBUG, "\t\t[%s fb%d %lld] cas orig(%lx) expected(%lx) target(%lx)", __FUNCTION__, 
        //     ctx->coro_id, boost::this_fiber::get_id(), swap_back, expected_value, target_value);
        if (swap_back == expected_value || swap_back == target_value) {
            ...
            // if (swap_back == target_value) {
            //     // only under recover situation can an operation read back his target value
            //     assert(ctx->req_type == KV_REQ_RECOVER_UNCOMMITTED);
            //     // print_log(DEBUG, "\t\t[%s] recover found", __FUNCTION__);
            // }
            win_addr_cnt[expected_value] ++;
            // num_success ++;
            // construct a heart-beat rw request
            KVRWAddr hb_addr;
            fill_heartbeat_addr(ctx->kv_modify_bk_0_cas_list[i].server_id, &hb_addr);
            ctx->kv_modify_bk_1_heartbeat_list.push_back(hb_addr);
            continue;   
        }
        ...
        // check value if failed
        uint64_t swap_value_addr = HashIndexConvert40To64Bits((*(RaceHashSlot *)&swap_back).pointer);
        // kv_assert(swap_value_addr != target_addr);
        if (swap_value_addr < target_addr) {
            min_target_addr = swap_value_addr;
        }
    }

    int num_success = win_addr_cnt[expected_value];
    if (num_success == num_idx_rep_ - 1) {
        ctx->consensus_state = KV_CONSENSUS_WIN_ALL;
        return;
    }
    if (num_success > (num_idx_rep_ - 1) / 2) {
        ctx->consensus_state = KV_CONSENSUS_WIN_MAJOR;
        return;
    }
    if (check_cas_consensus_is_no_win(win_addr_cnt) && min_target_addr == target_addr) {
        ctx->consensus_state = KV_CONSENSUS_WIN_LITTLE;
        return;
    }
    
    ctx->consensus_state = KV_CONSENSUS_FAIL;
    return;
}

int Client::check_cas_consensus_1(KVReqCtx * ctx) {
    int ret = 0;
    uint64_t target_value = ctx->kv_modify_bk_1_cas_list[0].swap_value;     // the swap target
    uint64_t expected_value = ctx->kv_modify_bk_1_cas_list[0].orig_value;   // the swap original value
    uint32_t num_success = 0;
...
void Client::kv_log_commit(KVReqCtx * ctx) {
    ...
    uint8_t ctl_bits = ((KVLogHeader *)ctx->kv_info->l_addr)->ctl_bits | KV_LOG_COMMITTED;
    IbvSrList * log_commit_sr_list = gen_log_commit_sr_lists(ctx->coro_id, ctl_bits, ctx->log_commit_addr_list, &log_commit_sr_list_num);
...
void Client::kv_update_bg_operations(KVReqCtx * ctx) {
    // TODO: conduct background operations
    mm_->mm_free_key(ctx->key_str);
    return;
}

void Client::kv_delete_read_buckets(KVReqCtx * ctx) {
    int ret = 0;
    KVLogHeader * header = (KVLogHeader *)ctx->kv_info->l_addr;

    // 1. generate send requests
    // print_log(DEBUG, "\t[%s fb%d %lld] 1. read blocks", __FUNCTION__, ctx->coro_id, boost::this_fiber::get_id());
    uint32_t read_bucket_sr_list_num;
    IbvSrList * read_bucket_sr_list = gen_read_bucket_sr_lists(ctx, &read_bucket_sr_list_num);
    // kv_assert(read_bucket_sr_list != NULL);
...
void Client::kv_delete_read_buckets_sync(KVReqCtx * ctx) {
    int ret = 0;
    KVLogHeader * header = (KVLogHeader *)ctx->kv_info->l_addr;
...
void Client::kv_delete_read_kv(KVReqCtx * ctx) {
    ...
    // 0. check cache
    if (ctx->use_cache && ctx->is_local_cache_hit) {
        // print_log(DEBUG, "\t[%s fb%d %lld] 0. check cache", __FUNCTION__, ctx->coro_id, boost::this_fiber::get_id());
        KVLogHeader * cached_kv_header = (KVLogHeader *)ctx->local_cache_addr;
...
void Client::kv_delete_bg_operations(KVReqCtx * ctx) {
    // TODO: conduct background operations
    return;
}
...
int Client::load_kv_requests(const char * fname, uint32_t st_idx, int32_t num_ops) {
    ...
    for (int i = 0; i < st_idx + num_local_operations_; i ++) {
        ...
        uint32_t all_len = sizeof(KVLogHeader) + strlen(key_buf) + strlen(value_buf);
...
void Client::update_kv_header(KVLogHeader * header, ClientMMAllocCtx * mm_alloc_ctx) {
...
void Client::init_kv_req_ctx(KVReqCtx * req_ctx, KVInfo * kv_info, char * operation) {
    ...
    req_ctx->kv_modify_bk_1_heartbeat_list.resize(num_idx_rep_ - 1);
...
int Client::client_recovery() {
    ...
    KVLogHeader * header = recover_log_info.local_header_addr;
    ...
    uint32_t total_len = sizeof(KVLogHeader) + header->key_length + header->value_length;
    ...
    rec_kv_info.key_len   = header->key_length;
    rec_kv_info.value_len = header->value_length;
...
int Client::kv_recover_update(KVReqCtx * ctx) {
    ...
    kv_recover_update_bg_operations(ctx);
    return ctx->ret_val.ret_code;
}
...
void Client::kv_recover_update_bg_operations(KVReqCtx * ctx) {
    // TODO: conduct background operations
    return;
}
...
void Client::get_addr_translate_table_n(const std::vector<uint8_t> & server_id_list) {
    int ret = 0;
    int healthy_meta_server = 0;
    uint32_t healthy_rkey = 0;
    ...
    healthy_rkey = server_mr_info_map_[healthy_meta_server]->rkey;

    ClientMetaAddrInfo * meta_addr_buf = (ClientMetaAddrInfo *)local_buf_;
    uint32_t lkey = local_buf_mr_->lkey;
    for (size_t i = 0; i < server_id_list.size(); i ++) {
        uint64_t remote_addr = remote_global_meta_addr_ + CLIENT_META_LEN * (server_id_list[i] - num_memory_ + 1) + sizeof(ClientLogMetaInfo);
        // printf("%d remote_addr: %lx\n", server_id_list[i], remote_addr);

        while (true) {
            ret = nm_->nm_rdma_read_from_sid_sync((void *)meta_addr_buf, lkey, sizeof(ClientMetaAddrInfo), 
                remote_addr, healthy_rkey, healthy_meta_server);
            if (ret != 0) {
                printf("read error\n");
            }
            if (meta_addr_buf->meta_info_type == TYPE_KVBLOCK) {
                // printf("find: %d %lx -> %d %lx\n", meta_addr_buf->server_id_list[0], meta_addr_buf->addr_list[0],
                //     meta_addr_buf->server_id_list[1], meta_addr_buf->addr_list[1]);
                // for (int j = 1; j < num_replication_; j ++) {
                //     printf("\t-> %d %lx\n", meta_addr_buf->server_id_list[j], meta_addr_buf->addr_list[j]);
                // }
                // ClientMetaAddrInfo client_meta;
                // memcpy(&client_meta, meta_addr_buf, sizeof(ClientMetaAddrInfo));
                meta_addr_info_.push_back(*meta_addr_buf);
            } else if (meta_addr_buf->meta_info_type == 0) {
                break;
            }
            remote_addr += sizeof(ClientMetaAddrInfo);
        }
    }
}

void Client::get_addr_translate_table(const std::vector<uint8_t> & server_id_list) {
    int ret = 0;
    int healthy_meta_server = 0;
    uint32_t healthy_rkey = 0;
    for (healthy_meta_server = 0; healthy_meta_server < num_replication_ && server_crash_map_[healthy_meta_server] == true; healthy_meta_server ++);
    healthy_rkey = server_mr_info_map_[healthy_meta_server]->rkey;
    // void * tmp_buf = malloc(CLIENT_META_LEN);
    // IbInfo ib_info;
    // nm_->get_ib_info(&ib_info);
    // struct ibv_mr * tmp_mr = ibv_reg_mr(ib_info.ib_pd, tmp_buf, CLIENT_META_LEN, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
    for (size_t i = 0; i < server_id_list.size(); i ++) {
        ...
        // find the first meta replication server that alive
        // print_log(DEBUG, "found healthy: %d", healthy_meta_server);
        print_log(DEBUG, "reading from server(%d) addr(%lx) rkey(%x)", healthy_meta_server, remote_addr, server_mr_info_map_[healthy_meta_server]->rkey);

        // printf("%d cnt: %d\n", server_id_list[i], cnt);
        // printf("size: %d\n", sizeof(ClientMetaAddrInfo));
        ret = nm_->nm_rdma_read_from_sid_sync((void *)local_addr, lkey, CLIENT_META_LEN / 1024, 
            remote_addr, server_mr_info_map_[healthy_meta_server]->rkey, healthy_meta_server);
        assert(ret == 0);

        // int cnt = 0;
        // remote_addr += sizeof(ClientLogMetaInfo);
        // ClientMetaAddrInfo * gt_info = (ClientMetaAddrInfo *)((uint64_t)tmp_buf + sizeof(ClientLogMetaInfo));
        // while (true) {
        //     memset((void *)local_addr, 0, sizeof(ClientMetaAddrInfo));
        //     ret = nm_->nm_rdma_read_from_sid_sync((void *)local_addr, lkey, sizeof(ClientMetaAddrInfo), 
        //         remote_addr, healthy_rkey, healthy_meta_server);
        //     if (memcmp(gt_info, (void *)local_addr, sizeof(ClientMetaAddrInfo)) != 0) {
        //         printf("ERROR!\n");
        //     }
        //     remote_addr += sizeof(ClientMetaAddrInfo);
        //     gt_info += 1;
        //     ClientMetaAddrInfo * meta_addr_info = (ClientMetaAddrInfo *)local_addr;
        //     if (meta_addr_info[0].meta_info_type == TYPE_KVBLOCK) {
        //         meta_addr_info_.push_back(meta_addr_info[0]);
        //         cnt ++;
        //     }
        //     if (meta_addr_info[0].meta_info_type == 0) {
        //         break;
        //     }
        // }
        ...
        for (int k = 0; addr_info_list[k].meta_info_type != 0; k ++) {
            print_log(DEBUG, "type: %d", addr_info_list[k].meta_info_type);
...
void * client_mm_daemon_thread(void * arg) {
    // TODO: finish this
    // check watermark and alloc new page
    return NULL;
}

void * client_mm_gc_thread(void * arg) {
    // TODO: finish this
    // 1. sync local gc info to remote
    // 2. gc self memory
    return NULL;
}