...
typedef struct TagKVReqCtx {
    ...
    std::vector<KVRWAddr>  kv_modify_bk_1_heartbeat_list; // heart beat to the modified backups
...
private:
    ...
    IbvSrList * gen_log_commit_sr_lists(uint32_t coro_id, uint8_t ctl_bits, std::vector<KVRWAddr> & rw_addr_list, __OUT uint32_t * num_sr_lists);
    void        free_log_commit_sr_lists(IbvSrList * sr_lists, int num_sr_lists);
    IbvSrList * gen_update_prev_sr_lists(KVReqCtx * ctx, __OUT uint32_t * num_sr_lists);   // TODO: remove this
    void        free_update_prev_sr_lists(IbvSrList * sr_lists, int num_sr_lists);         // TODO: remove this
    ...
    bool check_cas_consensus_is_no_win(const std::map<uint64_t, int> & win_addr_map);
    ...
    void kv_delete_read_buckets(KVReqCtx * ctx);
    void kv_delete_read_buckets_sync(KVReqCtx * ctx);
    ...
    void kv_recover_update_bg_operations(KVReqCtx * ctx);
...
public:
    ...
    void get_addr_translate_table_n(const std::vector<uint8_t> & server_id_list);
