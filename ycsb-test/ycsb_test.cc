...
int load_test_cnt_ops_mt_on_crash_cont_sample(Client & client, WorkloadFileName * workload_fnames) {
    ...
    for (int k = 0; k < 20; k ++) {
        ...
        if (k == 10) {
            ...
            for (int i = 0; i < 128; i ++) {
                server_id_list.push_back(i + num_memory);
            }
            gettimeofday(&st, NULL);
            client.get_addr_translate_table_n(server_id_list);
            gettimeofday(&et, NULL);
            reduce_ms = (int)(((et.tv_sec - st.tv_sec) * 1000000 + (et.tv_usec - st.tv_usec)) / 1000);
            printf("reducing %d\n", reduce_ms);
        }
        boost::fibers::fiber fb_list[client.num_coroutines_];
        for (int i = 0; i < client.num_coroutines_; i ++) {
